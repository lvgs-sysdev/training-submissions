...
export const initMypage = async (userId) => {
 ...
 */
const setupActionButton = (profile, userId) => {
 ...
 * 編集イベントの登録
 */
const setupMypageEvents = (userId) => {
    const form = document.getElementById("edit-profile-form");
    const imageInput = document.getElementById("edit-image");
    imageInput?.addEventListener("change", () => {
        const file = imageInput.files?.[0];
        if (file)
            MypageView.renderImagePreview(file);
    });
    ...
    document.getElementById("close-edit-modal").onclick = () => MypageView.hideEditModal();
