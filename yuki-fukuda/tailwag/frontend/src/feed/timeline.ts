...
export const refreshTimeline = async () => {
  ...
  if (listContainer) listContainer.innerHTML = "<p>わんこを探しています...</p>";
 ...
 * モーダル表示のためのイベント設定
 */
const setupPostModalEvents = (container: HTMLElement, posts: Post[]) => {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (icon) icon.innerHTML = result.liked ? "❤️" : "🐾";
