...
await fastify.register(session, {
  secret: process.env.SESSION_SECRET, // 32文字以上の秘密の鍵
  cookie: {
    secure: false,
    httpOnly: true,
...
fastify.get("/articles", async (request, reply) => {
  ...
  const [rows] = await connection.execute(
    `SELECT * FROM articles ORDER BY created_at DESC LIMIT ${ARTICLE_DISPLAY_LIMIT}`,
  );
...
fastify.post("/login", async (request, reply) => {
  ...
  const isAuthenticated =
    user && user.password && (await argon2.verify(user.password, password));
  if (!isAuthenticated) {
    return reply
      .status(401)
      .send({ error: "ユーザーIDまたはパスワードが違います" });
  }
